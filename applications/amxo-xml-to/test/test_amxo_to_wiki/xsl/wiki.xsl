...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <xsl:text>{|border=1&#10;</xsl:text>
    <xsl:text>|- style="background: lightgrey" align="left"&#10;</xsl:text>
...
==== Types ====
  ...
  <xsl:template match="odl:object">
    <xsl:text>|- style="background-color: rgb(255, 255, 153);"&#10;</xsl:text>
    <xsl:text>|</xsl:text>
  ...
  <xsl:template match="odl:parameter">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:if test="odl:validator">
          <xsl:text>&#60;br&#62;'''legal values''':</xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="odl:update-description">
        <xsl:text>'''on modify''':</xsl:text>
        <xsl:value-of select="odl:update-description"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text> - </xsl:text>
      </xsl:otherwise>
  ...
  <xsl:template match="odl:function">
    ...
    <xsl:text>''</xsl:text><xsl:value-of select="odl:return/@odl:type"/><xsl:text>'' </xsl:text>
  ...
  <xsl:template match="odl:type">
     <xsl:text>===== </xsl:text><xsl:value-of select="@odl:name"/><xsl:text> =====
</xsl:text>
  ...
  <xsl:template match="odl:member">
    <xsl:text>* ''</xsl:text>
    <xsl:value-of select="@odl:type"/><xsl:text>'' '''</xsl:text><xsl:value-of select="@odl:name"/>
    <xsl:text>''':</xsl:text>
     <xsl:call-template name="description"/>
     <xsl:text> 
</xsl:text>
